...
=======
      calculate: async (input, objectId) => {
        ...
        const hvacTracing = HVACTracing.getInstance();

        const traceId = hvacTracing.traceDuctSizing(

          {

            cfm: input.airflow || 1000,

            velocity: 800, // Default velocity

            material: input.material || input.duct_type || 'rectangular'
...
=======
              ...
              hvacTracing.endCalculationTrace(traceId, backendResult.results);
...
=======
          ...
          hvacTracing.endCalculationTrace(traceId, clientResult.results);
          ...
          hvacTracing.endCalculationTrace(traceId, undefined, [error instanceof Error ? error.message : 'Unknown error']);
