...
export class CenterlineTo3DConverter {
  ...
  private createFitting(
    ...
    const fitting: DuctFitting = {
      ...
      pressureLossCoefficient: this.getDefaultPressureLossCoefficient(insertionPoint.type),
      fittingGeometry: this.getDefaultFittingGeometry(insertionPoint.type, insertionPoint.angle),
  ...
  /**
   * Get default pressure loss coefficient for fitting type
   */
  private getDefaultPressureLossCoefficient(fittingType: string): number {
    ...
    return coefficients[fittingType] || 0.5;
  }

  /**
   * Get default fitting geometry for fitting type
   */
  private getDefaultFittingGeometry(fittingType: string, angle: number): any {
    const geometry: Record<string, any> = {
      'elbow': { bendRadius: 1.5, angle: angle || 90 },
      'transition': { reductionRatio: 1.0 },
      'tee': { branchAngle: 90 },
      'wye': { branchAngle: 45 },
      'reducer': { reductionRatio: 0.75 },
      'cap': {}
    };
    return geometry[fittingType] || {};
  }
