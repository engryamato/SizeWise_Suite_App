...
describe('FittingLossCalculator', () => {
  ...
  describe('Rectangular Fittings', () => {
    test('should calculate rectangular elbow with aspect ratio', () => {
      const config: FittingConfiguration = {
        type: '90deg_rect_smooth',
        ductShape: 'rectangular',
        width: 12,
        height: 8,
        parameter: '1.0' // radius ratio
      };

      const result = FittingLossCalculator.calculateFittingLoss(config, 1000);
      
      expect(result.kFactor).toBeGreaterThan(0);
      expect(result.fittingType).toBe('90deg_rect_smooth');
    });
  });
