  ...
  async runHVACIntegrationTest(config: TestConfig): Promise<void> {
    ...
    for (const projectUuid of projects) {
      for (let i = 0; i < config.iterations / 5; i++) {
        ...
        const fullCalculationResult = {
          success: true,
          input_data: calculationData.parameters as any,
          results: {
            area: calculationData.results.ductSize * calculationData.results.ductSize * Math.PI / 4, // Convert ductSize to area
            velocity: calculationData.results.velocity,
            pressure_loss: calculationData.results.pressureDrop,
            diameter: calculationData.results.ductSize
          },
          warnings: [],
          errors: []
        };
