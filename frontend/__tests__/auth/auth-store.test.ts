...
describe('Auth Store', () => {
  ...
  describe('Login Flow', () => {
    it('should handle successful login', async () => {
      ...
      mockHybridAuthManager.login.mockResolvedValueOnce({
        success: true,
        user: mockUser,
        token: 'jwt-token-123',
      })

      mockHybridAuthManager.getTierStatus.mockResolvedValueOnce(mockTierStatus)
  ...
  describe('Registration Flow', () => {
    it('should handle successful registration with trial', async () => {
      ...
      mockHybridAuthManager.register.mockResolvedValueOnce({
        success: true,
        user: mockUser,
        token: 'jwt-token-456',
      })
  ...
  describe('Tier Status Management', () => {
    ...
    it('should handle offline tier status', async () => {
      mockHybridAuthManager.getTierStatus.mockRejectedValueOnce(new Error('Network error'))
    ...
    it('should check action permissions', async () => {
      mockHybridAuthManager.canPerformAction.mockResolvedValueOnce(true)
      ...
      expect(mockHybridAuthManager.canPerformAction).toHaveBeenCalledWith('create_project', undefined)
    })
