          ...
          <div className="flex items-center space-x-2">
            <motion.div
              animate={{ 
                ...
                rotate: systemSummary.complianceStatus === 'pending' ? [0, 360] : 0
              }}
              transition={{ 
                duration: warnings.length > 0 ? 1 : 2,
                repeat: warnings.length > 0 || systemSummary.complianceStatus === 'pending' ? Infinity : 0
              }}
        ...
        <div className="flex items-center space-x-6">
          ...
          <div className="flex items-center space-x-2">
            <div className={`px-2 py-1 rounded text-xs font-medium ${
              systemSummary.complianceStatus === 'compliant' 
                ? 'bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200'
                : systemSummary.complianceStatus === 'non-compliant'
                ? 'bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200'
                : 'bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200'
            }`}>
              SMACNA {systemSummary.complianceStatus === 'compliant' ? '✓' : systemSummary.complianceStatus === 'non-compliant' ? '✗' : '...'}
            </div>
          </div>
        </div>
      </div>
